...
export default function Kontakt() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Przygotowanie danych do API (/api/email)
    const payload = {
      type: 'contact',
          ...
          {!submitted ? (
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="grid md:grid-cols-2 gap-6 text-left mt-8">
                <input
                  type="hidden"
                  name="_subject"
                  value="Nowa wiadomość z formularza kontaktowego – Luisówka"
                />
                <input
                  type="hidden"
                  name="Typ formularza"
                  value="Formularz kontaktowy na stronie Luisówka"
                />
            ...
            >
              <h2 className="text-xl md:text-2xl font-semibold !text-[#fdfbf7] p-6 rounded-xl">
                ...
                <br />
                Miło nam, że chcesz odwiedzić Luisówkę!
              </h2>
            </motion.div>
          )}
        </AnimatePresence>
